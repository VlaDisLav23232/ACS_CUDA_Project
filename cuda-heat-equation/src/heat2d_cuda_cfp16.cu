...
__global__ void heat2d_cfp16_kahan_reg_kernel(
    cfp16_t* u_a, cfp16_t* u_b, float* c_a, float* c_b,
    int N, float r, int T)
{
    ...
    for (int t = 0; t < T; t++) {
             ...
             idx += gridDim.x * blockDim.x)
        {
            ...
            int j = idx / N;
            if (i >= R && i < N - R && j >= R && j < N - R) {
                float center = cfp16_to_float(u_src[idx]) + c_src[idx];
                float lap = 2.0f * d_coeffs_cfp16[0] * center;
                for (int m = 1; m <= R; m++) {
                    int xm = j * N + (i - m);
                    int xp = j * N + (i + m);
                    int ym = (j - m) * N + i;
                    int yp = (j + m) * N + i;
                    lap += d_coeffs_cfp16[m] * (
                        (cfp16_to_float(u_src[xm]) + c_src[xm]) +
                        (cfp16_to_float(u_src[xp]) + c_src[xp]) +
                        (cfp16_to_float(u_src[ym]) + c_src[ym]) +
                        (cfp16_to_float(u_src[yp]) + c_src[yp]));
                }
                float exact = center + r * lap;
                cfp16_t stored = float_to_cfp16(exact);
                u_dst[idx] = stored;
                float stored_back = cfp16_to_float(stored);
                c_dst[idx] = exact - stored_back;
            }
        }
        grid.sync();
        for (int idx = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        grid.sync();
        cfp16_t* tmp_u = u_src; u_src = u_dst; u_dst = tmp_u;
...
StencilResult run_cuda_cfp16_kahan_reg(const StencilConfig& cfg) {
    ...
    cfp16_t *d_u_a, *d_u_b;
    float *d_c_a, *d_c_b;
    CUDA_CHECK(cudaMalloc(&d_u_a, cfp16_bytes));
    CUDA_CHECK(cudaMalloc(&d_u_b, cfp16_bytes));
    CUDA_CHECK(cudaMalloc(&d_c_a, float_bytes));
    CUDA_CHECK(cudaMalloc(&d_c_b, float_bytes));
    CUDA_CHECK(cudaMemcpy(d_u_a, h_data.data(), cfp16_bytes, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_u_b, h_data.data(), cfp16_bytes, cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &max_blocks_per_sm, heat2d_cfp16_kahan_reg_kernel, block_size, 0));
    int device;
    ...
    CUDA_CHECK(cudaLaunchCooperativeKernel(
        (void*)heat2d_cfp16_kahan_reg_kernel,
    ...
    cfp16_t* d_final_u = (T % 2 == 0) ? d_u_a : d_u_b;
    float* d_final_c = (T % 2 == 0) ? d_c_a : d_c_b;

    CUDA_CHECK(cudaMemcpy(h_data.data(), d_final_u, cfp16_bytes, cudaMemcpyDeviceToHost));
    ...
    double bytes_per_step = 2.0 * n_elems * sizeof(cfp16_t) + 2.0 * n_elems * sizeof(float);
    double total_bytes = bytes_per_step * cfg.timesteps;
    double bw = total_bytes / (elapsed_ms / 1000.0) / 1e9;

    StencilResult res;
    res.variant_name = "cuda_cfp16_kahan_reg";
    res.grid_size = N;
    res.dim = cfg.dim;
    res.stencil_reach = R;
    res.timesteps = cfg.timesteps;
